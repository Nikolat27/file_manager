<template>
    <div class="flex flex-col gap-y-6 pl-100 pt-32">
        <span class="text-[22px] font-bold">All files</span>

        <table
            class="w-[75%] h-auto border border-blue-200 rounded-xl overflow-hidden"
        >
            <thead>
            ...
            <tbody>
                ...
                <tr v-for="file in files" :key="file?.id">
                    ...
                    <td
                        @click="goToFile(filesShortUrls[file.id])"
                        v-if="filesShortUrls"
                        class="px-4 py-2 cursor-pointer text-blue-500 hover:text-blue-700 font-semibold"
                    >
                        click
                    </td>
...
let filesShortUrls = reactive([]);

function getAll() {
    axiosInstance
        .get(`/api/user/search?q=${route.query.q}`)
        .then((resp) => {
            files.value = resp.data.files;
            folders.value = resp.data.folders;
            filesShortUrls = resp.data.shortUrls;
        })
        .catch((err) => {
            showError(err.response?.data.error);
        });
